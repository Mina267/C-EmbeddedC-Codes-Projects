...
string_ReverseSentence3(char *sentence)
{
    int first_word = 0;
    int second_word = 0;
    int last_space = 1;
    ...
    for (int i = 0; sentence[i] ; i++)
    {
        if (sentence[i] == ' ' || sentence[i + 1] == '\0')
        {
            ...
            //string_copy(sentence, tmp, second_word, last_space - 1);
            //string_copy(sentence, sentence, second_word, last_space - 1);

            first_word = second_word + 1;
            second_word = last_space + 1;
            last_space = i;
            cnt++;
            printf("1st: %d\n", first_word);
            printf("2nd: %d\n", second_word);
...
void string_findAndReplacea(char *main_string, char* find, char* replace, char *new_string)
{
    bool isSame = true;
    int start = 0;
    int end = 0;
    ...
    for(int i = 0; main_string[i]; i++)
    {
        ...
        if (main_string[i] == ' ' || main_string[i + 1] == '\0')
        {
            start = end;
            end = i;
            printf("1st: %d\n", start);
